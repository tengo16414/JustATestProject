...
namespace Vuforia
{
	public abstract class VirtualButtonAbstractBehaviour : MonoBehaviour
	{
		...
		internal GameObject PreviousParent
		{
			get
			{
		...
		public bool CalculateButtonArea(out Vector2 topLeft, out Vector2 bottomRight)
		{
			...
			Vector2 a = new Vector2(vector[0] * num, vector[2] * num);
			Vector2 b = Vector2.Scale(new Vector2(base.transform.lossyScale[0], base.transform.lossyScale[2]) * 0.5f, new Vector2(1f, -1f));
			topLeft = a - b;
			bottomRight = a + b;
			return true;
		}
		...
		internal bool SetPosAndScaleFromButtonArea(Vector2 topLeft, Vector2 bottomRight)
		{
			ImageTargetAbstractBehaviour imageTargetBehaviour = this.GetImageTargetBehaviour();
			if (imageTargetBehaviour == null)
			{
			...
			Vector3 a = new Vector3(vector2[0], (vector2[0] + vector2[1]) * 0.5f, vector2[1]);
			base.transform.position = imageTargetBehaviour.transform.TransformPoint(position);
			base.transform.localScale = a / base.transform.parent.lossyScale[0];
			return true;
		}
