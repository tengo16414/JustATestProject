	...
	public abstract class DataSetTrackableBehaviour : TrackableBehaviour, WorldCenterTrackableBehaviour, IEditDataSetBehaviour
	{
		...
		protected virtual void OnDrawGizmos()
		{
			if (this.mInitializeSmartTerrain && this.mReconstructionToInitialize != null && !this.mAutoSetOccluderFromTargetSize)
			{
				...
				{
					Vector3 b = base.gameObject.transform.rotation * this.mSmartTerrainOccluderOffset;
					Gizmos.matrix = Matrix4x4.TRS(base.gameObject.transform.position + b, base.gameObject.transform.rotation * this.mSmartTerrainOccluderRotation, Vector3.one);
				}
				else
				{
				...
				Vector3 vector = this.mSmartTerrainOccluderBoundsMax - this.mSmartTerrainOccluderBoundsMin;
				Gizmos.DrawWireCube((this.mSmartTerrainOccluderBoundsMin + this.mSmartTerrainOccluderBoundsMax) / 2f, vector);
				float num = (vector.x + vector.y + vector.z) / 2f;
		...
		internal void SetDefaultOccluderBounds()
		{
			Vector3 vector;
			Vector3 vector2;
			this.CalculateDefaultOccluderBounds(out vector, out vector2);
		...
		internal static string GetDataSetName(string datasetPath)
		{
