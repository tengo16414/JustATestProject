...
namespace Vuforia.EditorClasses
{
	...
	public class ImageTargetEditor : Editor
	{
		...
		internal static void UpdateAspectRatio(SerializedImageTarget it, Vector2 size)
		{
			...
			using (System.Collections.Generic.List<ImageTargetAbstractBehaviour>.Enumerator enumerator = it.GetBehaviours().GetEnumerator())
			{
		...
		public static void EditorConfigureTarget(ImageTargetAbstractBehaviour itb, SerializedImageTarget serializedObject)
		{
			...
			if (VuforiaUtilities.GetPrefabType(itb) == 1)
			{
				return;
			}
			...
			{
				if (!serializedObject.InitializedInEditor && !EditorApplication.get_isPlaying())
				{
		...
		{
			ImageTargetAbstractBehaviour arg_22_0 = (ImageTargetAbstractBehaviour)base.get_target();
			this.mSerializedObject = new SerializedImageTarget(base.get_serializedObject());
		...
		{
			if ((ImageTargetAbstractBehaviour)base.get_target() != null)
			{
				if (this.mSerializedObject.ImageTargetType == ImageTargetType.PREDEFINED)
				{
		...
		{
			using (System.Collections.Generic.List<ImageTargetAbstractBehaviour>.Enumerator enumerator = serializedObject.GetBehaviours().GetEnumerator())
			{
				while (enumerator.MoveNext())
				{
					GameObject gameObject = enumerator.Current.gameObject;
					MeshFilter component = gameObject.GetComponent<MeshFilter>();
					if ((component == null || component.sharedMesh == null) && !serializedObject.AspectRatioProperty.get_hasMultipleDifferentValues())
					{
						ImageTargetEditor.UpdateMesh(gameObject, serializedObject.AspectRatio);
					}
				}
			}
			if (!serializedObject.DataSetPathProperty.get_hasMultipleDifferentValues() && !serializedObject.TrackableNameProperty.get_hasMultipleDifferentValues())
			{
				ImageTargetEditor.UpdateMaterial(serializedObject);
			}
		}
		...
		private void DrawCloudRecoTargetInspectorUI(bool typeChanged)
		{
			...
			if (expr_7B)
			{
				EditorGUILayout.HelpBox("Extended Tracking cannot be enabled at the same time as Smart Terrain.", 1);
			}
			GUI.enabled = !expr_7B;
		...
		private void DrawUserDefinedTargetInspectorUI(bool typeChanged)
		{
			...
			{
				EditorGUILayout.HelpBox("Target name must not exceed 64 character limit!", 3);
			}
		...
		private void DrawPredefinedTargetInspectorUI(bool typeChanged)
		{
			...
			{
				if (VuforiaUtilities.DrawDatasetTrackableInspector(this.mSerializedObject, typeChanged, new System.Func<ConfigData, string[]>(ImageTargetEditor.GetTrackableNames), "Image Target") | typeChanged)
				{
					if (base.get_serializedObject().get_isEditingMultipleObjects())
					{
						this.mSerializedObject.SerializedObject.ApplyModifiedPropertiesWithoutUndo();
						UnityEngine.Object[] targetObjects = this.mSerializedObject.SerializedObject.get_targetObjects();
