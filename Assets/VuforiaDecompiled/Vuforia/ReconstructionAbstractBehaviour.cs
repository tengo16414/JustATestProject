...
namespace Vuforia
{
	public abstract class ReconstructionAbstractBehaviour : MonoBehaviour
	{
		...
		public SurfaceAbstractBehaviour AssociateSurface(SurfaceAbstractBehaviour templateBehaviour, Surface newSurface)
		{
			...
			if (templateBehaviour != null)
			{
				if (templateBehaviour.transform.parent == base.transform)
				{
					...
					{
						if (VuforiaManager.Instance.WorldCenter == templateBehaviour)
						{
		...
		internal void ScaleEditorMeshesByFactor(float scaleFactor)
		{
		...
		internal void ScaleEditorPropPositionsByFactor(float scaleFactor)
		{
			if (Application.isPlaying)
			{
				return;
			...
			for (int i = 0; i < componentsInChildren.Length; i++)
			{
				componentsInChildren[i].gameObject.transform.localPosition *= scaleFactor;
			}
		}
