...
namespace Vuforia
{
	internal class LegacyHideExcessAreaClipping : IExcessAreaClipping
	{
		...
		public void Update(Vector3 planeOffset)
		{
			...
			Vector3 vector = this.mCamera.transform.InverseTransformPoint(ray.GetPoint(distance));
			plane.Raycast(ray2, out distance);
			Vector3 arg_228_0 = this.mCamera.transform.InverseTransformPoint(ray2.GetPoint(distance));
			Quaternion rotation = Quaternion.Inverse(this.mBgPlane.transform.localRotation) * Quaternion.AngleAxis(270f, Vector3.right);
			Vector3 position = this.mBgPlane.transform.TransformPoint(rotation * new Vector3(-1f, 0f, -1f));
			Vector3 position2 = this.mCamera.WorldToScreenPoint(position);
			Ray ray3 = this.mCamera.ScreenPointToRay(position2);
			Vector3 position3 = this.mBgPlane.transform.TransformPoint(rotation * new Vector3(1f, 0f, 1f));
			Vector3 position4 = this.mCamera.WorldToScreenPoint(position3);
			Ray ray4 = this.mCamera.ScreenPointToRay(position4);
			plane.Raycast(ray3, out distance);
			Vector3 vector2 = this.mCamera.transform.InverseTransformPoint(ray3.GetPoint(distance));
			plane.Raycast(ray4, out distance);
			Vector3 vector3 = this.mCamera.transform.InverseTransformPoint(ray4.GetPoint(distance));
			float num2 = vector2.x - vector.x;
			float num3 = arg_228_0.y - vector.y;
			this.mLeftPlane.transform.localPosition = new Vector3(vector.x + num2 * 0.5f, vector.y + num3 * 0.5f, num) + planeOffset;
			this.mLeftPlane.transform.localScale = new Vector3(num2, num3, 1f);
			float num4 = arg_228_0.x - vector3.x;
			float num5 = arg_228_0.y - vector.y;
			this.mRightPlane.transform.localPosition = new Vector3(vector3.x + num4 * 0.5f, vector.y + num5 * 0.5f, num) + planeOffset;
			this.mRightPlane.transform.localScale = new Vector3(num4, num5, 1f);
			float num6 = vector3.x - vector2.x;
			float num7 = arg_228_0.y - vector3.y;
			this.mTopPlane.transform.localPosition = new Vector3(vector2.x + num6 * 0.5f, vector3.y + num7 * 0.5f, num) + planeOffset;
			this.mTopPlane.transform.localScale = new Vector3(num6, num7, 1f);
			float num8 = vector3.x - vector2.x;
			float num9 = vector2.y - vector.y;
			this.mBottomPlane.transform.localPosition = new Vector3(vector2.x + num8 * 0.5f, vector.y + num9 * 0.5f, num) + planeOffset;
			this.mBottomPlane.transform.localScale = new Vector3(num8, num9, 1f);
