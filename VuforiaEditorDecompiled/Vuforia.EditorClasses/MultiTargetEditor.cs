...
namespace Vuforia.EditorClasses
{
	...
	public class MultiTargetEditor : Editor
	{
		...
		internal static void UpdateParts(SerializedMultiTarget serializedObject, ConfigData.MultiTargetPartData[] prtConfigs)
		{
			foreach (MultiTargetAbstractBehaviour current in serializedObject.GetBehaviours())
			{
				...
				else
				{
					...
					{
						MultiTargetEditor.mPlaneMesh = new Mesh
						{
					...
					using (System.Collections.Generic.List<GameObject>.Enumerator enumerator2 = list.GetEnumerator())
					{
		...
		public static void EditorConfigureTarget(MultiTargetAbstractBehaviour mtb, SerializedMultiTarget serializedObject)
		{
			...
			if (VuforiaUtilities.GetPrefabType(mtb) == 1)
			{
				return;
			}
			...
			using (serializedObject.Edit())
			{
				if (!serializedObject.InitializedInEditor && !EditorApplication.get_isPlaying())
				{
					...
					System.Collections.Generic.List<ConfigData.MultiTargetPartData> parts = multiTargetData.parts;
		...
		{
			MultiTargetAbstractBehaviour arg_22_0 = (MultiTargetAbstractBehaviour)base.get_target();
			this.mSerializedObject = new SerializedMultiTarget(base.get_serializedObject());
			MultiTargetEditor.EditorConfigureTarget(arg_22_0, this.mSerializedObject);
		}

		public void OnSceneGUI()
		{
			TrackableBehaviour trackableBehaviour = (TrackableBehaviour)base.get_target();
		...
		public override void OnInspectorGUI()
		{
			...
			if (ConfigDataManager.Instance.NumConfigDataObjects > 1)
			{
				...
				{
					flag = VuforiaUtilities.DrawDatasetTrackableInspector(this.mSerializedObject, false, new System.Func<ConfigData, string[]>(MultiTargetEditor.GetTrackableNames), "Multi Target");
