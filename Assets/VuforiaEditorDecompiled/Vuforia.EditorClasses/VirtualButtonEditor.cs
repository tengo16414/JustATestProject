...
namespace Vuforia.EditorClasses
{
	...
	public class VirtualButtonEditor : Editor
	{
				...
				for (int j = 0; j < componentsInChildren.Length; j++)
				{
					if (componentsInChildren[j].VirtualButtonName == vbs[i].name)
					{
						...
						using (serializedObject.Edit())
						{
							...
							VirtualButtonEditor.GetSensitivityProperty(serializedObject).set_enumValueIndex((int)vbs[i].sensitivity);
		...
		public static void EditorConfigureTarget(VirtualButtonAbstractBehaviour vb)
		{
			...
			if (VuforiaUtilities.GetPrefabType(vb) == 1)
			{
				return;
			}
		...
		{
			VirtualButtonAbstractBehaviour arg_2D_0 = (VirtualButtonAbstractBehaviour)base.get_target();
			this.mName = VirtualButtonEditor.GetNameProperty(base.get_serializedObject());
			this.mSensitivity = VirtualButtonEditor.GetSensitivityProperty(base.get_serializedObject());
		...
		{
			VirtualButtonAbstractBehaviour virtualButtonAbstractBehaviour = (VirtualButtonAbstractBehaviour)base.get_target();
		...
		private static void AddVirtualButton(ImageTargetAbstractBehaviour it, ConfigData.VirtualButtonData vb)
		{
			...
			if (virtualButtonAbstractBehaviour != null)
			{
				...
				using (serializedObject.Edit())
				{
					VirtualButtonEditor.GetSensitivityProperty(serializedObject).set_enumValueIndex((int)vb.sensitivity);
				}
