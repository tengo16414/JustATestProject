...
namespace Vuforia
{
	internal class StateManagerImpl : StateManager
	{
		...
		{
			Quaternion rhs = Quaternion.Inverse(camToTargetPose.orientation);
			Vector3 position = trackable.transform.rotation * Quaternion.AngleAxis(90f, Vector3.left) * rhs * -camToTargetPose.position + trackable.transform.position;
			Quaternion rotation = trackable.transform.rotation * Quaternion.AngleAxis(90f, Vector3.left) * rhs;
