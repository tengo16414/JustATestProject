...
namespace Vuforia.EditorClasses
{
	internal class VideoBackgroundEditor : ConfigurationEditor
	{
		...
		public override void DrawInspectorGUI()
		{
			...
			HideExcessAreaAbstractBehaviour.CLIPPING_MODE cLIPPING_MODE = (HideExcessAreaAbstractBehaviour.CLIPPING_MODE)this.mClippingMode.get_enumValueIndex();
			EditorGUILayout.PropertyField(this.mClippingMode, new GUIContent("Overflow geometry"), new GUILayoutOption[0]);
			HideExcessAreaAbstractBehaviour.CLIPPING_MODE enumValueIndex = (HideExcessAreaAbstractBehaviour.CLIPPING_MODE)this.mClippingMode.get_enumValueIndex();
			if (cLIPPING_MODE != enumValueIndex)
			{
				cLIPPING_MODE = enumValueIndex;
				this.mMatteShader.set_objectReferenceValue(VuforiaAbstractConfiguration.VideoBackgroundConfiguration.GetDefaultMatteShader(cLIPPING_MODE));
			}
