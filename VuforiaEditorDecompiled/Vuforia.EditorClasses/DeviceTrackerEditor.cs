...
namespace Vuforia.EditorClasses
{
	internal class DeviceTrackerEditor : ConfigurationEditor
	{
		...
		{
			bool boolValue = this.mAutoInitTracker.get_boolValue();
			EditorGUILayout.PropertyField(this.mAutoInitTracker, new GUIContent("Enable device pose tracking"), new GUILayoutOption[0]);
			if (this.mAutoInitTracker.get_boolValue() != boolValue)
			{
				boolValue = this.mAutoInitTracker.get_boolValue();
				this.mAutoStartTracker.set_boolValue(boolValue);
			}
			if (boolValue)
			{
				...
				RotationalDeviceTracker.MODEL_CORRECTION_MODE enumValueIndex = (RotationalDeviceTracker.MODEL_CORRECTION_MODE)this.mModelCorrectionMode.get_enumValueIndex();
				if (enumValueIndex != RotationalDeviceTracker.MODEL_CORRECTION_MODE.NONE)
				{
					...
					if (this.mModelTransformEnabled.get_boolValue())
					{
						this.mModelTransform.set_vector3Value(EditorGUILayout.Vector3Field("Pivot point", this.mModelTransform.get_vector3Value(), new GUILayoutOption[0]));
						if (GUILayout.Button(new GUIContent("Set default pivot point", "Reset pivot point to default value of this model correction mode"), new GUILayoutOption[0]))
						{
							...
							{
								this.mModelTransform.set_vector3Value(DeviceTrackerARController.DEFAULT_HEAD_PIVOT);
								return;
							}
							...
							this.mModelTransform.set_vector3Value(DeviceTrackerARController.DEFAULT_HANDHELD_PIVOT);
						}
					}
				}
			}
