...
namespace Vuforia
{
	...
	public abstract class VideoBackgroundAbstractBehaviour : MonoBehaviour
	{
		...
		public void ApplyStereoDepthToMatrices()
		{
			...
			Vector3 position = base.transform.TransformPoint(Vector3.zero);
			float arg_4D_0 = -VuforiaManager.Instance.ARCameraTransform.InverseTransformPoint(position).x;
		...
		private void OnPostRender()
		{
			if (this.mVuforiaARController.HasStarted && base.isActiveAndEnabled)
			{
				...
				{
					RenderTexture targetTexture = base.GetComponent<Camera>().targetTexture;
					RenderBuffer colorBuffer = targetTexture ? targetTexture.colorBuffer : Display.main.colorBuffer;
					RenderBuffer depthBuffer = targetTexture ? targetTexture.depthBuffer : Display.main.depthBuffer;
					Graphics.SetRenderTarget(colorBuffer, depthBuffer);
					VuforiaWrapper.Instance.SetRenderBuffers(colorBuffer.GetNativeRenderBufferPtr());
				}
				VideoBackgroundAbstractBehaviour.mRenderCounter--;
