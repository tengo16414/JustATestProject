	...
	internal static class SmartTerrainInitializationTargetEditorExtension
	{
		...
		internal static void DrawInspectorForInitializationTarget(SerializedDataSetTrackable serializedDst, bool sizeOnlyKnownAtRuntime)
		{
			...
			if (serializedDst.InitializeSmartTerrain)
			{
				...
				{
					EditorGUILayout.HelpBox("Press 'NEW' to instantiate a new Smart Terrain prefab in the scene.", 0);
				}
				...
				if (!sizeOnlyKnownAtRuntime || !serializedDst.AutoSetOccluderFromTargetSize)
				{
					...
					{
						EditorGUILayout.HelpBox("Check this box to automatically set the occluder box depending on the cloud target size at runtime.", 0);
					...
					EditorGUILayout.HelpBox("Use the surface offset below if the primary surface is not co-planar with the initialization target", 0);
					...
					{
						if (SmartTerrainInitializationTargetEditorExtension.sTransformHandle != SmartTerrainInitializationTargetEditorExtension.TransformHandle.NONE && Tools.get_current() != -1 && Tools.get_current() != null)
						{
		...
		internal static void DrawSceneGUIForInitializationTarget(SerializedDataSetTrackable serializedDst, bool sizeOnlyKnownAtRuntime)
		{
			...
			using (serializedDst.Edit())
			{
				...
				{
					using (System.Collections.Generic.List<GameObject>.Enumerator enumerator = serializedDst.GetGameObjects().GetEnumerator())
					{
						while (enumerator.MoveNext())
						{
							...
							if (SmartTerrainInitializationTargetEditorExtension.sTransformHandle == SmartTerrainInitializationTargetEditorExtension.TransformHandle.TRANSLATION)
							{
								Handles.set_matrix(Matrix4x4.TRS(transform.position, transform.rotation, Vector3.one));
							...
							if (SmartTerrainInitializationTargetEditorExtension.sTransformHandle == SmartTerrainInitializationTargetEditorExtension.TransformHandle.ROTATION)
							{
								Vector3 b = transform.rotation * serializedDst.SmartTerrainOccluderOffset;
								Handles.set_matrix(Matrix4x4.TRS(transform.position + b, transform.rotation, Vector3.one));
								Quaternion smartTerrainOccluderRotation = serializedDst.SmartTerrainOccluderRotation;
							...
							Handles.set_matrix(Matrix4x4.identity);
						}
					}
				}
			}
