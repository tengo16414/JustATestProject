		...
		private void PositionVideoMesh()
		{
			...
			if (this.mVuforiaBehaviour != null)
			{
				...
				else
				{
					...
					Vector3 localPosition = this.mCamera.transform.InverseTransformPoint(ray.GetPoint(distance));
					base.gameObject.transform.localPosition = localPosition;
					base.gameObject.transform.position += this.mBackgroundOffset;
					float num2 = 1f / this.mCamera.projectionMatrix[5];
					float num3 = this.mStereoDepth * num2;
					float num4 = num3 * num;
					...
					{
						num3 *= videoBackgroundRectInViewPort.height / (float)this.mViewHeight;
					}
					if ((int)videoBackgroundRectInViewPort.width != this.mViewWidth)
					{
						num4 *= videoBackgroundRectInViewPort.width / (float)this.mViewWidth;
					}
					float num5;
					if (VuforiaRuntimeUtilities.IsPortraitOrientation)
					{
						num5 = (float)this.mTextureInfo.imageSize.y / (float)this.mTextureInfo.imageSize.x;
					}
					else
					{
						num5 = (float)this.mTextureInfo.imageSize.x / (float)this.mTextureInfo.imageSize.y;
					}
					if (!float.IsNaN(num5))
					{
						if (this.ShouldFitWidth())
						{
							if (num5 > 1f)
							{
								base.gameObject.transform.localScale = new Vector3(num4, 1f, num4 / num5);
							}
							else
							{
								base.gameObject.transform.localScale = new Vector3(num4 / num5, 1f, num4);
							}
						}
						else if (num5 > 1f)
						{
							base.gameObject.transform.localScale = new Vector3(num3 * num5, 1f, num3);
						}
						else
						{
							base.gameObject.transform.localScale = new Vector3(num3, 1f, num3 * num5);
						}
					}
				}
			}
