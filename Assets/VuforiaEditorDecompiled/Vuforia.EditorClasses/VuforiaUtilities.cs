	...
	internal class VuforiaUtilities
	{
		...
		public static bool OrientationFromStringArray(out Quaternion result, string[] valuesToParse)
		{
			...
			try
			{
				...
				{
					result = Quaternion.AngleAxis(num, axis);
				}
				else
				{
					...
					result = Quaternion.AngleAxis(57.29578f * num, axis);
				}
			}
			catch
			{
		...
		{
			if (VuforiaUtilities.GetPrefabType(target) == 1)
			{
		...
		public static ConfigData.ImageTargetData CreateDefaultImageTarget()
		{
			return new ConfigData.ImageTargetData
			{
				...
				virtualButtons = new System.Collections.Generic.List<ConfigData.VirtualButtonData>()
			};
		}

		public static bool DrawDatasetTrackableInspector(SerializedDataSetTrackable serializedObject, bool resetTrackable, System.Func<ConfigData, string[]> getTrackableNamesFunc, string trackableLabel)
		{
			bool result = false;
			...
			int num = -1;
			if (!serializedObject.DataSetPathProperty.get_hasMultipleDifferentValues())
			{
			...
			if (num2 >= 0)
			{
				...
				int num3 = -1;
				if (!serializedObject.TrackableNameProperty.get_hasMultipleDifferentValues())
				{
		...
		{
			if (System.IO.File.Exists(pathWithoutExtension + ".png"))
			{
				pathWithExtension = pathWithoutExtension + ".png";
				return true;
			}
			if (System.IO.File.Exists(pathWithoutExtension + ".jpg"))
			{
				pathWithExtension = pathWithoutExtension + ".jpg";
				return true;
			}
			if (System.IO.File.Exists(pathWithoutExtension + ".jpeg"))
			{
