...
namespace Vuforia.EditorClasses
{
	...
	public class VuMarkEditor : Editor
	{
		...
		internal static void UpdateAspectRatio(SerializedVuMark serializedObject, Vector2 size)
		{
			...
			using (System.Collections.Generic.List<VuMarkAbstractBehaviour>.Enumerator enumerator = serializedObject.GetBehaviours().GetEnumerator())
			{
		...
		private static void CheckMesh(SerializedVuMark serializedObject)
		{
			...
			if (!serializedObject.PreviewImageProperty.get_hasMultipleDifferentValues())
			{
				VuMarkEditor.UpdateMaterial(serializedObject);
				return;
			}
			UnityEngine.Object[] targetObjects = serializedObject.SerializedObject.get_targetObjects();
		...
		private static void UpdateMesh(GameObject gameObject, Rect boundingBox, Vector2 origin)
		{
			...
			Vector3 b = new Vector3(-origin.x, 0f, -origin.y);
			Vector3 vector = num * (new Vector3(boundingBox.xMin, 0f, boundingBox.yMin) + b);
			Vector3 vector2 = num * (new Vector3(boundingBox.xMin, 0f, boundingBox.yMax) + b);
			Vector3 vector3 = num * (new Vector3(boundingBox.xMax, 0f, boundingBox.yMin) + b);
			Vector3 vector4 = num * (new Vector3(boundingBox.xMax, 0f, boundingBox.yMax) + b);
		...
		public static void EditorConfigureTarget(VuMarkAbstractBehaviour vmb, SerializedVuMark serializedObject)
		{
			...
			if (VuforiaUtilities.GetPrefabType(vmb) == 1)
			{
				return;
			}
		...
		{
			VuMarkAbstractBehaviour arg_22_0 = (VuMarkAbstractBehaviour)base.get_target();
			this.mSerializedObject = new SerializedVuMark(base.get_serializedObject());
		...
		public override void OnInspectorGUI()
		{
			...
			if (ConfigDataManager.Instance.NumConfigDataObjects > 1)
			{
				...
				{
					if (VuforiaUtilities.DrawDatasetTrackableInspector(this.mSerializedObject, false, new System.Func<ConfigData, string[]>(VuMarkEditor.GetTrackableNames), "VuMark Template"))
					{
						ConfigData.VuMarkData vuMarkData;
