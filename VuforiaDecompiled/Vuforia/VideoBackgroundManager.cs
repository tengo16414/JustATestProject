	...
	public class VideoBackgroundManager : ARController, IVideoBackgroundEventHandler
	{
		...
		internal void Initialize()
		{
			...
			else
			{
				int width = 1280;
				int height = 720;
				IntPtr intPtr = VuforiaRenderer.Instance.createNativeTexture(width, height, 16);
				if (intPtr != IntPtr.Zero)
				{
					this.mTexture = Texture2D.CreateExternalTexture(width, height, TextureFormat.RGBA32, true, true, intPtr);
				}
			}
