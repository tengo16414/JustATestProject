	...
	public class DistortionRenderingBehaviour : MonoBehaviour, IVideoBackgroundEventHandler
	{
		...
		private void OnEnable()
		{
			...
			for (int j = 0; j < this.mStereoCameras.Length; j++)
			{
				...
				this.mStereoCameras[j].cullingMask &= ~(1 << this.mRenderLayer);
			}
		...
		private void CreateRenderTexture()
		{
			for (int i = 0; i < this.mTextures.Length; i++)
			{
				...
				if (!(this.mTextures[i] != null) || num != this.mTextures[i].width || num2 != this.mTextures[i].height)
				{
					...
					this.mTextures[i] = new RenderTexture(num, num2, 24, RenderTextureFormat.Default)
					{
		...
		private Camera SetupOrthographicCamera()
		{
			...
			float left = -(1f + 2f * (viewport.xMin / num));
			float right = 1f - 2f * (viewport.xMax / num - 1f);
			float bottom = -(1f + 2f * (viewport.yMin / num2));
			float top = 1f - 2f * (viewport.yMax / num2 - 1f);
			camera.orthographic = true;
			camera.projectionMatrix = Matrix4x4.Ortho(left, right, bottom, top, -1f, 1f);
			...
			for (int i = 0; i < array.Length; i++)
			{
				...
				num3 = Math.Max(num3, camera2.depth);
			}
			camera.depth = num3 + 10f;
			return camera;
		}
