...
namespace Vuforia.EditorClasses
{
	...
	public class CylinderTargetEditor : Editor
	{
		...
		private static Material[] UpdateMaterials(string dataSetName, string trackableName, bool hasBottomGeometry, bool hasTopGeometry, bool insideMaterial, Material[] oldMaterials)
		{
			...
			if (!System.IO.File.Exists(str + ".Body_scaled.png") && !System.IO.File.Exists(str + ".Body_scaled.jpg") && !System.IO.File.Exists(str + ".Body_scaled.jpeg"))
			{
		...
		public static void EditorConfigureTarget(CylinderTargetAbstractBehaviour ctb, SerializedCylinderTarget serializedObject)
		{
			...
			if (VuforiaUtilities.GetPrefabType(ctb) == 1)
			{
				return;
			}
		...
		{
			CylinderTargetAbstractBehaviour arg_22_0 = (CylinderTargetAbstractBehaviour)base.get_target();
			this.mSerializedObject = new SerializedCylinderTarget(base.get_serializedObject());
		...
		public override void OnInspectorGUI()
		{
			...
			if (ConfigDataManager.Instance.NumConfigDataObjects > 1)
			{
				...
				{
					if (VuforiaUtilities.DrawDatasetTrackableInspector(this.mSerializedObject, false, new System.Func<ConfigData, string[]>(CylinderTargetEditor.GetTrackableNames), "Cylinder Target"))
					{
						ConfigData.CylinderTargetData cylinderTargetData;
		...
		{
			if ((CylinderTargetAbstractBehaviour)base.get_target() != null)
			{
