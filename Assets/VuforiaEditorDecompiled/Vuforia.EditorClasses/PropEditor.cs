...
namespace Vuforia.EditorClasses
{
	...
	public class PropEditor : Editor
	{
		...
		public static void EditorConfigureTarget(PropAbstractBehaviour prop, SerializedProp serializedObject)
		{
			...
			if (VuforiaUtilities.GetPrefabType(prop) == 1)
			{
				return;
			}
		...
		{
			PropAbstractBehaviour arg_22_0 = (PropAbstractBehaviour)base.get_target();
			this.mSerializedObject = new SerializedProp(base.get_serializedObject());
		...
		public override void OnInspectorGUI()
		{
			...
			{
				EditorGUILayout.HelpBox("This is a prop template that will be duplicated at runtime for newly found smart terrain props. The DefaultSmartTerrainEventHandler will use the same template for every prop at runtime. You can implement a custom ISmartTerrainEventHandler to create a different behavior.", 1);
				EditorGUILayout.HelpBox("The mesh filter, mesh collider and/or box collider selected below will be automatically updated with new revisions of the smart terrain prop. Set them to None to ignore updates.", 0);
