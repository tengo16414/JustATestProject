...
namespace Vuforia
{
	internal class StencilHideExcessAreaClipping : IExcessAreaClipping
	{
		...
		public void Update(Vector3 planeOffset)
		{
			...
			Vector3 expr_F7 = this.mCamera.transform.InverseTransformPoint(ray2.GetPoint(distance));
			float num2 = expr_F7.x - vector.x;
			float num3 = expr_F7.y - vector.y;
			this.mClippingPlane.transform.localPosition = new Vector3(vector.x + num2 * 0.5f, vector.y + num3 * 0.5f, num) + planeOffset;
			this.mClippingPlane.transform.localScale = new Vector3(num2 * 1.5f, num3 * 1.5f, 1f);
		}
	}
}
